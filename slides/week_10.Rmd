...
Our **model** has random variates at each node of the network,
correlated with neighbors.
...
truth <- list(
              alpha = 2.5,
              rho = 0.6)
truth$covmat <- truth$alpha^2 * (diag(nrow(A)) + truth$rho * A)
```
...
2. Suppose the network is *known*, and use it as a *smoothing prior*
   to help infer noisy or unobserved values at some of the nodes.
